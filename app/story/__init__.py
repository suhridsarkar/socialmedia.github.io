...
from app.models import post, user, postLike,friendrequests,notification, db, selectFriend, Action, fileType, notificationStatus, notificationIsFor, story, storyNew
...
def story():
    ...
    if session.get('uid'):
        ...
        return render_template('story.html',title ="Story", friendsList = friendsList, user = userdata)
    else:
        return redirect('/welcome')
    
@story_bp.route("/addstory")
def addstory():
    ...
    if session.get('uid'):
        ...
        return render_template('addStory.html',title ="Add Story", friendsList = friendsList, user = userdata)
    else:
        return redirect('/welcome')


@story_bp.route("/saveStory", methods=['POST','GET'])
def saveStory():
    ...
    if file.filename == '':
        ...
        if file and allowed_file(file.filename):
            ...
            file.save(os.path.join(current_app.config['UPLOAD_FOLDER']+"story", filename))
    ...
    postData = storyNew(text=text, userId=session.get('uid'),fileType = file_type, file=filename, seeYourPost=seeYourPost, ip = client_ip_address, date = date, isActive = Action.Y)
...
@story_bp.route("/seePost", methods=['POST'])
def seePost():
...
# Like Insert Data
# # # # # # # # # # # # # # # # # #
@story_bp.route("/postLikeInsert", methods=['POST'])
def postLikeInsert():
    userId = session.get('uid')
